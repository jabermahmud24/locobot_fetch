...
class RobotSerial(Robot):
    ...
    def __init__(self, dh_params, dh_type="normal", analytical_inv=None
                 ...
                 , inv_m="jac_pinv", step_size=0.1, max_iter=200, final_loss=1e-3):
    ...
    def ts(self):
        ...
        #         for row in matrix:
        #             writer.writerow(row)
            # print(f"Matrix {i} saved to {file_name}")
        return ts
    ...
    def axis_frames(self):
        ...
        jacobian = np.zeros([6, self.num_axis])
        # if self.dh_type == "normal":
        jacobian[0:3, 0] = np.cross(np.array([0., 0., 1.]), fs[-1].t_3_1.reshape([3, ]))
        jacobian[3:6, 0] = np.array([0., 0., 1.])
        for i in range(1, self.num_axis):
        # for i in range(1, self.num_axis):
            jacobian[0:3, i] = np.cross(fs[i-1].z_3_1.reshape([3, ]), (fs[-1].t_3_1 - fs[i-1].t_3_1).reshape([3, ]))
            jacobian[3:6, i] = fs[i-1].z_3_1.reshape([3, ])
            # print("jacobian inside robot serial")
            # print(jacobian)
            
            filename = f"jacobian_matrix{1}.csv"
            np.savetxt(filename, jacobian, delimiter=",")
    ...
    def jacobian(self):
        axis_fs = self.axis_frames
        jacob = np.zeros([6, self.num_axis])
        if self.dh_type == "normal":
            jacob[0:3, 0] = np.cross(np.array([0., 0., 1.]), axis_fs[-1].t_3_1.reshape([3, ]))
            jacob[3:6, 0] = np.array([0., 0., 1.])
            for i in range(1, self.num_axis):
                jacob[0:3, i] = np.cross(axis_fs[i-1].z_3_1.reshape([3, ]), (axis_fs[-1].t_3_1 - axis_fs[i-1].t_3_1).reshape([3, ]))
                jacob[3:6, i] = axis_fs[i-1].z_3_1.reshape([3, ])
        if self.dh_type == "modified":
            for i in range(0, self.num_axis):
                jacob[0:3, i] = np.cross(axis_fs[i].z_3_1.reshape([3, ]), (axis_fs[-1].t_3_1 - axis_fs[i].t_3_1).reshape([3, ]))
                jacob[3:6, i] = axis_fs[i].z_3_1.reshape([3, ])
    ...
    # @property
    # def jacobian_forward(self, end_frame):
    #     axis_fs = self.axis_frames
    #     jacobian_forward = np.zeros([6, self.num_axis])
    #     if self.dh_type == "normal":
    #         jacobian_forward[0:3, 0] = np.cross(np.array([0., 0., 1.]), axis_fs[-1].t_3_1.reshape([3, ]))
    #         jacobian_forward[3:6, 0] = np.array([0., 0., 1.])
    #         for i in range(1, self.num_axis):
    #             jacobian_forward[0:3, i] = np.cross(axis_fs[i-1].z_3_1.reshape([3, ]), (axis_fs[-1].t_3_1 - axis_fs[i-1].t_3_1).reshape([3, ]))
    #             jacobian_forward[3:6, i] = axis_fs[i-1].z_3_1.reshape([3, ])
    #     if self.dh_type == "modified":
    #         for i in range(0, self.num_axis):
    #             jacobian_forward[0:3, i] = np.cross(axis_fs[i].z_3_1.reshape([3, ]), (axis_fs[-1].t_3_1 - axis_fs[i].t_3_1).reshape([3, ]))
    #             jacobian_forward[3:6, i] = axis_fs[i].z_3_1.reshape([3, ])
        
        # print("================================Jacobian matrix")
        #print(jacob)
        # return jacobian_forward
